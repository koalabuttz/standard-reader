...
- [x] GreenGale `#contentRef` two-phase fetch + `get_blob` for image blobs are wired into the pipeline. **`at.unthread.content`** (Unthread — a Markdown string in `content`) now has its own decoder, reusing the Markdown pipeline. *Still deferred:* Pckt `gallery` ref (a record fetch like contentRef).
