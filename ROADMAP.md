...
## Status — 2026-05-26: an interactive reader, with images

Workspace builds; the suite is green (48 tests across both crates: core unit + integration over real-record fixtures incl. an offline mock of the whole pipeline, the redb cache round-trip, and the TUI's renderer/state/`TestBackend` tests). `sr` launches a **`ratatui` reader** — add a blog by handle (a local follow-list persisted in redb), browse the sidebar → document list → reader, search, command palette, mouse. The reader is a **block-flow** that renders real inline + cover images (`ratatui-image`, iTerm2 on hterm), all over a worker thread with an offline cache. Reading needs no auth. OAuth and a layout picker are next.
...
- [x] **Inline + cover images** — the reader is now a **block-flow** (text runs + image segments, row-scrolled). Blob bytes fetched by the worker, cached in redb (offline), decoded (`image`) and encoded for the terminal via `ratatui-image` (iTerm2 on hterm, halfblocks fallback). Cover renders atop; images render when fully in view (encode-once, no scroll flicker). *Deferred:* smooth partial-image clipping at the viewport edge, `ThreadProtocol` off-thread encoding, AVIF, intra-paragraph inline images.
