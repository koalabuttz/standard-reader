...
- [x] ~~Author `basicTheme` toggle (uniform vs. author's styling).~~ **Dropped** in favour of user-driven customization (see the v0.1 "Customization" item): one consistent, user-controlled render path is leaner than honouring each publication's styling, and is what a reader wants.
